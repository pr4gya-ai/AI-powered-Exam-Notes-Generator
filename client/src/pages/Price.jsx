...
import { serverURL } from '../services/api.js'
...
function Pricing() {
  ...
  const handlePaying = async (amount) => {
    try {
      ...
      setPaying(true);
      const result = await axios.post(serverURL + "/api/credits/order", { amount }, 
      ...
      // If no URL is returned, mark as failed
      setPaying(false);
...
function PricingCard({ title, price,
  amount, credits, description, features, popular, selectedPrice, setSelectedPrice, onBuy, paying, payingAmount
}) {
  const isSelected = selectedPrice === amount;
  const isPlayingThisCard = paying && payingAmount === amount;
  ...
  `}
    >
      ...
      <button
        ...
        `}
      >
        {isPlayingThisCard ? "Processing..." : "Buy Now"}
