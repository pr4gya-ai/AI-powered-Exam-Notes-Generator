...
import { googleAuth, getCurrentUser } from '../services/api';
...
const Auth = () => {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      console.log("Firebase user:", { name: firebaseUser.displayName, email: firebaseUser.email });

      if (!firebaseUser.email) {
        throw new Error("Email not provided by Firebase");
      }

      let responseUser = await googleAuth({
        name: firebaseUser.displayName || "User",
        email: firebaseUser.email,
      });

      if (!responseUser) {
        responseUser = await getCurrentUser(dispatch);
      }
    ...
    <div className='min-h-screen overflow-hidden bg-white text-black px-8'>
      ...
      >
        ...
        <div className='flex items-center gap-3'>
          <img src='/assets/logo.jpg' alt='ExamNotes AI logo' className='h-10 w-10 object-contain rounded-full border border-white/20' />
